...
class Consensus extends EventEmitter {
  ...
  constructor(id, peers = [], logState) {
    ...
    // Automatically begin upon instantiation.
    this._init();
  }
   ...
   * 
   * @private
   * @returns {module:brig/consensus~Consensus}
   */ 
  _init() {
    ...
    this.state.on(RoleChanged, role => this.emit(RoleChanged, role));
  ...
  resetElectionTimer() {
    ...
    }, Consensus.getElectionTimeoutMs());
  }
   ...
   *
   * @param {module:brig/message~VoteRequestMessage} vReq - Vote request 
   * message.
   */
  handleVoteRequest(vReq) {
    ...
    const votedForNode = this.state.votedFor === p.id;
    ...
    if ((p.currentTerm === this.state.currentTerm) && logOk && votedForNode) {
      ...
      this.getPeer(p.peerId).write(new VoteResponseMessage({
        peerId: this.id,
        currentTerm: this.state.currentTerm,
        voteCasted: true
      }));
  ...
  handleVoteResponse(vRes) {
    ...
    const isCurrentTerm = vRes.currentTerm === this.state.currentTerm;
    ...
    if (isCandidate && isCurrentTerm && vRes.voteCasted) {
      this.state.votesReceived.add(vRes.peerId);
    }
    ...
    if (this.state.votesReceived.size >= minVotesToWin) {
      ...
      for (let i = 0; i < this.peers.length; i++) { 
        ...
        // own log.
        this.state.sentLength[vRes.peerId] = this.state.log.entries.length;
        // The node does not receive any ack from the follower.
        this.state.ackedLength[vRes.peerId] = 0;

        this.replicateLog(this.getPeer(this.id), this.getPeer(vRes.peerId));
      }
    } else if (vRes.currentTerm > this.state.currentTerm) {
      ...
      this.state.currentTerm = vRes.currentTerm;
      this.state.setCurrentRole(Follower);
      this.state.voteFor(null);
      this.resetElectionTimer();
    }
  ...
  replicateLog(leader, follower) {
    ...
    const prefixLength = this.state.sentLength[follwer.id];
   ...
   * to the leader.
   *
   * @param {object} payload - Payload to be included in log entry.
   * broadcasted.
   */
  handleBroadcast(bCast) {
    ...
    if (this.state.currentRole === Leader) {
      ...
      this.replicateLog(this.getPeer(this.id), this.getPeer(this.state.currentLeader));
    } else { 
      // If the node is not a leader, and it does not follow a 
      // leader, then it should buffer the message until it follows a leader.
      this.once(RoleChanged, (_role) => this.handleBroadcast(bCast));
    }
  }
  ...
  handleLogRequest(lReq) {
    ...
    if (p.term > this.state.currentTerm) {
      this.state.currentTerm = p.currentTerm;
      this.voteFor(null);
      this.resetElectionTimer();
    } 
    ...
    const logOk = (this.state.log.entries.length >= p.prefixLength) &&
      (p.prefixLength === 0 || 
        this.state.log[p.prefixLength - 1].term === prefixTerm);
    ...
    if (p.term === this.state.currentTerm && logOk) {
      ...
      // entries.
      const ack = prefixLength + suffix.length;
   ...
   * @param {Array.<module:brig/messages~LogEntry>} suffix - Entries to append.
   */
  appendEntries(prefixLength, leaderCommit, suffix) {
    ...
    if (suffix.length > 0 && this.state.log.entries.length > prefixLength) {
      ...
      let index = Math.min(this.state.log.length, 
  ...
  handleLogResponse(lRes) {
    ...
    if (r.term === this.state.currentTerm && this.state.currentRole === Leader) {
      if (r.success && r.ack >= this.state.ackedLength.get(r.follower)) {
        this.state.sentLength.set(r.follower, r.ack);
        this.state.ackedLength.set(r.follower, r.ack);
        this.commitLogEntries();
      } else if (this.state.sentLength.get(r.follower) > 0) {
        this.state.sentLength.set(r.follower, 
          this.state.sentLength.get(r.follower) - 1);
        this.replicateLog(this.getPeer(this.id), r.follower);
      }
