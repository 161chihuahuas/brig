...
class Cluster extends EventEmitter {
  ...
  static getElectionTimeoutMs() {
    return 500 + Math.ceil(Math.random() * 500);
  }

  static getReplicateTimeoutMs() {
    return 250 + Math.ceil(Math.random() * 250);
  }
  ...
  _connectPeerStream(peer) {
    ...
    if (peer.id === this.id) {
      throw new Error('Cannot setup circular stream.');
    }
  ...
  handleVoteRequest(vReq) {
    ...
    if (vReq.currentTerm > this.state.currentTerm) {
      this._dbg('VoteRequestMessage term is greater our current term.', 
        'Becoming a follower.')
