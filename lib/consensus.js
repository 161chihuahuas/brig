...
class Peer extends EventEmitter {
   ...
   *
   * @event module:brig/consensus~Peer#message_queued
   * @param {string} id - Peer ID.
   * @param {module:brig/messages~Message}
   */
  ...
  constructor(id, tx) {
    ...
    this.tx = (_id, msg) => {
      this.emit('message_queued', _id, msg);
      tx && tx(_id, msg);
      return this;
    };
  }
   ...
   * @fires module:brig/consensus~Peer#message_queued
...
class Cluster extends EventEmitter {
  ...
  static getElectionTimeoutMs() {
    return 150 + Math.ceil(Math.random() * 150);
  }
  ...
  createProtocolMapping() {
    ...
    const mapping = {
      [VoteRequestMessage.method]: (msg) => {
        return this.emit(VoteRequest, Message.from(msg));
      },
      [VoteResponseMessage.method]: (msg) => {
        return this.emit(VoteResponse, Message.from(msg));
      },
      [LogRequestMessage.method]: (msg) => {
        return this.emit(LogRequest, Message.from(msg));
      },
      [LogResponseMessage.method]: (msg) => {
        return this.emit(LogResponse, Message.from(msg));
      },
      [BroadcastMessage.method]: (msg) => {
        return this.emit(Broadcast, Message.from(msg));
      }
    };
  ...
  replicateLog(leaderId, follower) {
    ...
    if (prefixLength > 0) {
      let lastLogEntry = this.state.log.getEntryByIndex(prefixLength - 1);
  ...
  handleBroadcast(bCast) {
    ...
    if (this.state.currentRole === Leader || this.state.currentLeader === this.id) {
      ...
      this.state.log.append(new LogEntry(msg.payload, this.state.currentTerm));
